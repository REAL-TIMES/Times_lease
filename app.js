...
function KakaoMapView({ address, kakaoKey }) {
  ...
  // 좌표 조회
  useEffect(function() {
    if (!address || !kakaoKey) { setStatus('nokey'); return; }
    setStatus('loading');
    fetch('/api/geocode?address=' + encodeURIComponent(address))
      .then(function(r){ return r.json(); })
      .then(function(d){
        if (d.lat && d.lng) setCoords({ lat: parseFloat(d.lat), lng: parseFloat(d.lng) });
        else setStatus('error');
      })
      .catch(function(){ setStatus('error'); });
  }, [address, kakaoKey]);

  // 캔버스에 OSM 타일 합성 + 마커 렌더링
  useEffect(function() {
    if (!coords || !canvasRef.current) return;
    var canvas = canvasRef.current;
    var ctx = canvas.getContext('2d');
    var W = 260, H = 195, Z = 16;
    canvas.width = W; canvas.height = H;
    ...
    var px = ((lng + 180) / 360 * n - tx) * 256;
    var py = ((0.5 - Math.log((1 + sinLat) / (1 - sinLat)) / (4 * Math.PI)) * n - ty) * 256;
    var offX = W / 2 - px;
    var offY = H / 2 - py;
    ...
    for (var dx = -1; dx <= 1; dx++) {
      for (var dy = -1; dy <= 1; dy++) {
        (function(ddx, ddy){
          ...
          img.onload = function() {
            ctx.drawImage(img, (ddx + 1) * 256 + offX, (ddy + 1) * 256 + offY, 256, 256);
            loaded++;
            if (loaded === total) drawMarker();
          };
          img.onerror = function() {
            loaded++;
            if (loaded === total) drawMarker();
          };
          img.src = 'https://tile.openstreetmap.org/' + Z + '/' + (tx + ddx) + '/' + (ty + ddy) + '.png';
        })(dx, dy);
      }
    }
  }, [coords]);
  ...
  return (
    <div style={{width:'260px',height:'195px',position:'relative',background:'#f0ede6',overflow:'hidden'}}>
      <canvas ref={canvasRef}
        style={{width:'260px',height:'195px',display:status==='ok'?'block':'none'}} />
      {status!=='ok' && (
        <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:'8px'}}>
          <div style={{fontSize:'20px'}}>{status==='loading'?'🗺':'📍'}</div>
          <div style={{fontSize:'11px',color:'#aaa',textAlign:'center',padding:'0 12px'}}>
            {status==='loading'?'지도 불러오는 중…':status==='nokey'?'API 키를 입력하세요':'지도를 불러올 수 없습니다'}
          </div>
        </div>
      )}
      {status==='ok' && (
        <div style={{position:'absolute',bottom:0,left:0,right:0,background:'rgba(0,0,0,0.4)',padding:'3px 6px',fontSize:'9px',color:'white',textAlign:'center'}}>
          {address}
        </div>
      )}
    </div>
